...
/** Which Microsoft cloud the host belongs to (BC-020). M1 only emits `global`. */
export type Cloud = 'global' | 'gcc-high' | 'dod' | 'china' | 'unknown';

/** An opaque identifier carried by the link, such as a document GUID (M2). */
...
/** A best effort hint that is not a component, such as an item type code (M2). */
...
/** A wrapper removed before parsing, recorded in the order it was removed (M2). */
...
export interface Method {
  ...
  code: string;
  /** Plain language statement of how the result was obtained. */
  text: string;
}
...
export interface ParseSuccess {
  ...
  /** Detected link form, e.g. `library-view`. */
  form: string;
  method: Method;
  cloud: Cloud;
  /** Decoded server relative path of the item (file or folder). */
  path: string;
  /** Encoded URL of the containing folder (or the folder itself for folder results). */
  folderUrl: string;
  /** Encoded URL of the file. Absent for folder results. */
  fileUrl?: string;
